...
\begin{parts}
\begin{comment}
\part[1] ¿De cuántas formas podrán distribuirse 3 premios iguales entre diez aspirantes?  
\begin{solution} $ C_{10}^3=\frac{10!}{7!\cdot3!}=120$  \end{solution}
\part[1] ¿Y si los premios fueran diferentes? 

\begin{solution} $ V_{10}^3=\frac{10!}{7!}=720$  \end{solution}
\end{comment}
...
\question Calcula los dominios de las siguientes funciones usando la notación de conjuntos

\begin{parts}
\part[1] $f(x)=\dfrac{x^3-6x^2+4x+8}{x^3-x^2-9x+9}$ 
% g = (x**3-6*x**2+4*x+8)/(x**3-x**2-9*x+9)
% singularities(g, x)
\begin{solution} $\left\{-3, 1, 3\right\}$\end{solution}
\part[1] $f(x) =
\left\{
	\begin{array}{clc}
		4  & \mbox{si } & x < -2 \\
